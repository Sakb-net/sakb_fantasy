...
<div class="row">
    <div class="col-lg-12 margin-tb">
        <div class="pull-right">
            @if($groupEldwry_create > 0)
            <a class="btn btn-success fa fa-plus"  data-toggle="tooltip" data-placement="top" data-title="{{trans('app.add')}} {{trans('app.groupEldwry')}}" href="{{ route('admin.groupEldwry.create') }}"></a>
            @endif
        </div>
    </div>
</div>
...
<script>
   $(document).ready( function () {
    ...
    $('#groupEldwryTable').DataTable({
        ...
        ajax: "{{ route('admin.groupEldwry.list') }}",
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name'  },
            { data: 'user', name: "user"  },
