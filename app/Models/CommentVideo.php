...
class CommentVideo extends Model {
    ...
    protected $fillable = [
        'user_id', 'video_id', 'parent_one_id', 'parent_two_id',
    ...
    public function videos() {
        return $this->belongsTo(\App\Models\Post::class,'video_id');
    }
    ...
    public function parentOneID() {
        return $this->hasMany(\App\Models\CommentVideo::class, 'parent_one_id');
    }

    public function childrenstwo() {
        return $this->hasMany(\App\Models\CommentVideo::class, 'parent_two_id');
    }

    public function parentID() {
        return $this->belongsTo(\App\Models\CommentVideo::class, 'parent_one_id');
    }

    public function childrensID() {
        return $this->belongsTo(\App\Models\CommentVideo::class, 'parent_two_id');
    }
    ...
    public static function commentLink($colum = 'link', $val_colum = '', $is_active = 1, $type = '') {
        $data = static::with('videos')->where($colum, $val_colum)->where('is_active', $is_active);
        if (!empty($type)) {
    ...
    public static function commentParentOneTwo($name_colum='link',$val_colum='', $is_active = 1, $type = '') {
        if (!empty($val_colum)) {
            $parent_comment = static::commentLink($name_colum, $val_colum, $is_active, $type);
    ...
    public static function commentTypeACtive($video_id, $type = 'comment', $is_active = 1, $limit = 0, $colum = '', $val_colum = NULL, $myquestion = '', $colum_order = 'id', $type_order = 'DESC', $name_order = '') {
        //, 'childrenstwo.grandchildrentwo'
        $data = static::with('childrenstwo')->where('video_id', $video_id)->where('type', $type)->where('is_active', $is_active);
        if (!empty($myquestion)) {
...
//                $result = $data->withCount('parentOneID')->orderBy('parent_one_id',$type_order);
    ...
    public function insertComment($user_id, $user_image, $name, $email, $video_id, $type, $content, $parent_one_id = null, $image = "", $is_read = 0, $is_active = 0, $link = '',$parent_two_id= null) {
